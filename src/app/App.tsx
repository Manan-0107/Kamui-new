...
import {
  Play, Search, Bell, Star, BookmarkPlus,
  ChevronLeft, ChevronRight, Menu, X, Info,
  TrendingUp, Clock, Heart, Flame, Eye,
} from "lucide-react";
...
const TICKER_ITEMS = [
  "🔴 LIVE · Jujutsu Kaisen S2 EP23 now streaming",
  "⚡ 1.4M viewers watching Attack on Titan right now",
  "🆕 Demon Slayer: Hashira Training EP8 just dropped",
  "🔥 Bleach: TYBW Part 3 trending in 52 countries",
  "⭐ New season confirmed — Re:Zero Season 3",
  "🎌 Vinland Saga wins Best Anime of the Year",
  "🆕 Spy x Family Season 2 finale this Sunday",
  "💀 Chainsaw Man Season 2 officially greenlit",
];
...
// ─── Starfield data (deterministic positions) ────────────────────────────────
...
interface FeaturedShow {
  ...
  tags: string[]; rating: number; year: number; episodes: number;
  studio: string; viewers: number; image: string; accentColor: string;
}
interface AnimeItem  { id: number; title: string; sub: string; genre: string; rating: number; badge?: string | null; image: string; }
interface WatchItem  { id: number; title: string; sub: string; ep: string; progress: number; image: string; }
interface RankedItem { id: number; title: string; genre: string; year: number; rating: number; views: string; image: string; }

// ─── Data ────────────────────────────────────────────────────────────────────

const GENRES = ["All","Action","Romance","Sci-Fi","Fantasy","Horror","Comedy","Sports","Slice of Life"];

const FEATURED_SHOWS: FeaturedShow[] = [
  {
    title: "Attack on Titan", subtitle: "The Final Season — Part 3",
    ...
    tags: ["Dark Fantasy","Action","Drama"], rating: 9.9, year: 2023, episodes: 87, studio: "MAPPA", viewers: 1247832,
    image: "https://images.unsplash.com/photo-1536440136628-849c177e76a1?w=1600&h=900&fit=crop&auto=format", accentColor: "#e11d84",
  },
  {
    title: "Jujutsu Kaisen", subtitle: "Season 2 — Shibuya Incident",
    ...
    tags: ["Action","Supernatural","Horror"], rating: 9.1, year: 2023, episodes: 47, studio: "MAPPA", viewers: 982451,
    image: "https://images.unsplash.com/photo-1518709766631-a6a7f45921c3?w=1600&h=900&fit=crop&auto=format", accentColor: "#7c3aed",
  },
  {
    title: "Demon Slayer", subtitle: "Hashira Training Arc",
    ...
    tags: ["Action","Adventure","Shounen"], rating: 9.3, year: 2024, episodes: 8, studio: "ufotable", viewers: 1103774,
    image: "https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=1600&h=900&fit=crop&auto=format", accentColor: "#f97316",
  },
  {
    title: "Bleach: TYBW", subtitle: "The Thousand-Year Blood War — Part 3",
    ...
    tags: ["Action","Supernatural","Shounen"], rating: 9.0, year: 2024, episodes: 13, studio: "Pierrot", viewers: 876340,
    image: "https://images.unsplash.com/photo-1492515114975-b062d1a270ae?w=1600&h=900&fit=crop&auto=format", accentColor: "#06b6d4",
  },
];

const ALL_ANIME: AnimeItem[] = [
  ...
  { id:3,  title:"Bleach: TYBW",      sub:"Part 3",            genre:"Action",       rating:9.0, badge:"EP 13 NEW",  image:"https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=400&h=560&fit=crop&auto=format" },
...
const CONTINUE_WATCHING: WatchItem[] = [
...
function LiveViewers({ base }: { base: number }) {
...
function AnimatedBar({ progress }: { progress: number }) {
  ...
  useEffect(() => { const t = setTimeout(() => setW(progress), 450); return () => clearTimeout(t); }, [progress]);
  return (
...
function MarqueeTicker() {
  ...
  return (
    <div className="relative bg-secondary/50 border-y border-border overflow-hidden py-2.5 backdrop-blur-sm">
...
function PortraitCard({ item, index = 0 }: { item: AnimeItem; index?: number }) {
...
function LandscapeCard({ item }: { item: WatchItem }) {
  return (
    <motion.div
...
function ScrollRow({ title, icon, children }: { title: string; icon?: React.ReactNode; children: React.ReactNode }) {
  ...
  const scroll = (dir: "left"|"right") => ref.current?.scrollBy({ left: dir==="left" ? -340 : 340, behavior:"smooth" });
  return (
    <section>
      ...
      <div ref={ref} className="flex gap-4 overflow-x-auto pb-3" style={{ scrollbarWidth:"none", msOverflowStyle:"none" } as React.CSSProperties}>
        {children}
      </div>
    </section>
  );
...
function Nav({ onSearch, searchQuery }: { onSearch: (q:string)=>void; searchQuery: string }) {
      ...
      <div className="px-6 lg:px-16 h-16 flex items-center justify-between">
        <div className="flex items-center gap-10">
          <motion.div className="flex items-center cursor-pointer" whileHover={{ scale: 1.05 }} transition={{ type:"spring", stiffness:400 }}>
            ...
            >HADES</span>
            <span className="ml-2 text-[10px] font-bold tracking-widest text-accent/60 hidden sm:block" style={{ WebkitTextFillColor:"rgba(225,29,132,0.6)" }}>ANIME</span>
          </motion.div>

          <div className="hidden md:flex items-center gap-7 text-sm font-semibold">
            {["Home","Browse","Movies","Schedule","My List"].map(link => (
              <a key={link} href="#" className={`transition-colors hover:text-primary relative group ${link==="Home" ? "text-foreground" : "text-muted-foreground"}`}>
                {link}
        ...
        <div className="flex items-center gap-3">
          <AnimatePresence mode="wait">
            {searchOpen ? (
              <motion.div key="open" initial={{ width:0, opacity:0 }} animate={{ width:200, opacity:1 }} exit={{ width:0, opacity:0 }} transition={{ duration:0.22 }}
                ...
                <input autoFocus value={searchQuery} onChange={e => onSearch(e.target.value)}
                  placeholder="Search anime..." className="bg-transparent text-sm outline-none w-full text-foreground placeholder:text-muted-foreground"
                  onBlur={() => { setSearchOpen(false); onSearch(""); }} />
              </motion.div>
            ) : (
              <motion.button key="closed" initial={{ opacity:0 }} animate={{ opacity:1 }} onClick={() => setSearchOpen(true)}
          ...
          <button className="w-9 h-9 rounded-full bg-white/8 hover:bg-white/15 border border-white/10 flex items-center justify-center transition-colors backdrop-blur-sm relative">
            ...
            <motion.span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-accent rounded-full"
              animate={{ scale:[1,1.45,1] }} transition={{ repeat:Infinity, duration:1.8 }} />
          </button>

          <motion.div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center text-white text-xs font-bold cursor-pointer select-none"
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div initial={{ height:0, opacity:0 }} animate={{ height:"auto", opacity:1 }} exit={{ height:0, opacity:0 }}
            ...
            {["Home","Browse","Movies","Schedule","My List"].map(link => (
              <a key={link} href="#" className="text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors" onClick={() => setMenuOpen(false)}>{link}</a>
            ))}
          </motion.div>
        )}
      </AnimatePresence>
...
function Hero() {
  ...
  useEffect(() => {
    if (paused) return;
    const id = setInterval(() => setIdx(i => (i+1) % FEATURED_SHOWS.length), 7000);
    return () => clearInterval(id);
  }, [paused]);

  return (
    <div className="relative h-screen min-h-[640px] max-h-[940px] flex items-end"
      onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>

      {/* BG crossfade */}
      <AnimatePresence mode="sync">
        <motion.div key={idx} className="absolute inset-0 bg-card"
      ...
      {/* Accent color bloom */}
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] rounded-full opacity-[0.08] blur-[80px] pointer-events-none transition-all duration-1000"
      ...
      <div className="absolute bottom-8 right-8 lg:right-16 flex gap-2 z-20">
        {FEATURED_SHOWS.map((_, i) => (
          <button key={i} onClick={() => setIdx(i)}
            ...
            {i===idx && (
              <motion.div className="absolute inset-0 rounded-full" style={{ background: show.accentColor }}
                initial={{ scaleX:0, originX:0 }} animate={{ scaleX:1 }} transition={{ duration: paused ? 0 : 7, ease:"linear" }} />
            )}
          </button>
        ))}
      </div>
      ...
      {/* Prev / Next */}
      <button onClick={() => setIdx(i => (i-1+FEATURED_SHOWS.length)%FEATURED_SHOWS.length)}
      ...
      <button onClick={() => setIdx(i => (i+1)%FEATURED_SHOWS.length)}
      ...
      {/* Hero content */}
      <AnimatePresence mode="wait">
        <motion.div key={idx} className="relative z-10 px-6 lg:px-16 pb-16 lg:pb-20 max-w-2xl"
          initial={{ opacity:0, x:-28 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:18 }} transition={{ duration:0.5, ease:"easeOut" }}>

          <motion.div className="flex items-center gap-2 mb-4"
            animate={{ opacity:[0.7,1,0.7] }} transition={{ repeat:Infinity, duration:2.2 }}>
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {show.tags.map(tag => (
              <span key={tag} className="text-xs px-3 py-1 border rounded-full font-medium"
          ...
          <div className="flex flex-wrap items-center gap-3">
            <motion.button className="pulse-btn flex items-center gap-2.5 text-white px-8 py-3.5 rounded-full font-bold text-sm tracking-wide active:scale-95"
              style={{ background: show.accentColor }}
              whileHover={{ scale:1.05 }} whileTap={{ scale:0.96 }}>
              <Play className="w-4 h-4 fill-white" /> Watch Now
            </motion.button>
            <motion.button className="flex items-center gap-2 bg-white/10 hover:bg-white/18 text-white px-6 py-3.5 rounded-full font-semibold text-sm backdrop-blur-sm border border-white/15 transition-all"
              whileHover={{ scale:1.03 }} whileTap={{ scale:0.97 }}>
              <Info className="w-4 h-4" /> Details
            </motion.button>
            <motion.button className="w-11 h-11 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/15 flex items-center justify-center transition-all group"
...
function GenreSection({ active, setActive }: { active: string; setActive: (g:string)=>void }) {
  ...
  return (
    <section>
      ...
      <div className="flex flex-wrap gap-2 mb-7">
        {GENRES.map(g => (
          <motion.button key={g} onClick={() => setActive(g)}
            className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors duration-200 ${
              active===g ? "bg-primary text-white shadow-lg shadow-primary/30" : "bg-secondary text-muted-foreground hover:bg-primary/20 hover:text-primary border border-border"
            }`}
      ...
      <AnimatePresence mode="wait">
        <motion.div key={active} initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-8 }} transition={{ duration:0.22 }}
          ...
          {filtered.length > 0 ? filtered.map((item, i) => (
            <PortraitCard key={item.id} item={item} index={i} />
          )) : (
      ...
      <div className="space-y-3">
        {TOP_RATED.map((item, i) => (
          <motion.div key={item.id}
            ...
            initial={{ opacity:0, x:-20 }} animate={{ opacity:1, x:0 }} transition={{ delay: i*0.08, duration:0.35 }}
            whileHover={{ x:5 }}>
            <span className={`text-2xl font-bold w-8 text-center flex-none ${i===0?"text-yellow-400":i===1?"text-gray-300":i===2?"text-amber-600":"text-muted-foreground"}`}
              style={{ fontFamily:"'Rajdhani',sans-serif" }}>{String(i+1).padStart(2,"0")}</span>
...
function Footer() {
  return (
    <footer className="border-t border-border mt-20 px-6 lg:px-16 py-14">
      <div className="flex flex-col md:flex-row justify-between gap-10 mb-10">
        <div className="max-w-xs">
          <span className="text-2xl font-bold tracking-[0.18em] mb-3 inline-block"
      ...
      <div className="pt-6 border-t border-border flex flex-col sm:flex-row justify-between items-center gap-2 text-muted-foreground text-xs">
        <p>© 2024 Hades Anime Inc. All rights reserved.</p>
...
export default function App() {
  ...
  const filteredTrending = searchQuery
    ? TRENDING.filter(i => i.title.toLowerCase().includes(searchQuery.toLowerCase()))
    : TRENDING;
